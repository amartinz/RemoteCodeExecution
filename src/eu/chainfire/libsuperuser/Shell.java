/*
s * Copyright (C) 2012-2013 Jorrit "Chainfire" Jongma
 *
...
import net.openfiresecurity.remotecodeexecution.BuildConfig;
...
public class Shell {
	 ...
	 *            The commands to execute
	 * @param environment
	 *            List of all environment variables (in 'key=value' format) or
	 *            null for defaults
	 * @param wantSTDERR
	 *            Return STDERR in the output ?
	 * @return Output of the commands, or null in case of an error
	 */
	public static List<String> run(String shell, String[] commands,
			String[] environment, boolean wantSTDERR) {
		...
		if (BuildConfig.DEBUG) {
			...
			if ((Looper.myLooper() != null)
				...
				throw new ShellOnMainThreadException(
						ShellOnMainThreadException.EXCEPTION_COMMAND);
			}
	...
	public static class SH {
		...
		public static List<String> run(List<String> commands) {
			return Shell.run("sh",
					commands.toArray(new String[commands.size()]), null, false);
		}
	...
	public static class SU {
		...
		public static boolean available() {
			...
			List<String> ret = run(new String[] { "id", "echo -EOC-" });
			if (ret == null) {
				return false;
			}
		...
		public static String version(boolean internal) {
			...
			List<String> ret = Shell.run("sh", new String[] {
					internal ? "su -V" : "su -v", "exit" }, null, false);
			if (ret == null) {
				return null;
			}
	 ...
	 * 
	 * <p>
	 * The main thread must certainly has a Looper, thus if you call
	...
	public static class Interactive {
				...
				if (command.onCommandResultListener != null) {
					if (buffer != null) {
						if (handler != null) {
							...
							command.onCommandResultListener.onCommandResult(
									command.code, lastExitCode, buffer);
						}
					}
				}
		...
		/**
		 * Internal call that launches the shell, starts gobbling, and stars
		...
		private synchronized boolean open() {
			...
			try {
				...
				STDOUT = new StreamGobbler(shell.toUpperCase() + "-",
				...
				STDERR = new StreamGobbler(shell.toUpperCase() + "*",
		...
		public void close() {
			...
			// mode.
			if (!_idle && BuildConfig.DEBUG && (Looper.myLooper() != null)
					&& (Looper.myLooper() == Looper.getMainLooper())) {
		 ...
		 * 
		 * <p>
		 * In practise this means that in most simple cases all callbacks will
